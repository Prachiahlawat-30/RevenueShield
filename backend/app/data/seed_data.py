...
def seed_database(db: Session, reset: bool = False) -> Dict[str, int]:
    ...
    incident = PaymentIncident(
        ...
        root_cause_summary="Upstream processor network communication timeout surge on Gateway A.",
        confidence=Decimal("0.880"),
        evidence_list=[
            "✓ Gateway A timeout rate increased 4.2× (current: 18.2% vs baseline: 4.8%)",
            "✓ Gateway B and Gateway C remain healthy with <1.2% timeout rates",
            "✓ 81% of recent soft failures originate from Gateway A endpoint",
            "✓ Peak error frequency detected during the active business operating window",
        ],
