        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="space-y-1">
              <div className="flex items-center gap-2 flex-wrap">
                ...
                {status?.is_configured ? (
                  ...
                  <span className="rounded-md bg-amber-100 dark:bg-amber-950/50 border border-amber-300 dark:border-amber-500/30 px-2 py-0.5 text-[10px] font-mono font-semibold text-amber-800 dark:text-amber-200">
                    Mock / Demo Sandbox
                  </span>
                )}
              </div>
          ...
          <div className="flex items-center gap-2.5 self-start sm:self-auto flex-wrap">
            <button
              ...
              disabled={testing}
              className="inline-flex items-center gap-1.5 rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 hover:bg-slate-50 dark:hover:bg-slate-700/80 px-3 py-1.5 text-xs font-semibold text-slate-800 dark:text-slate-100 shadow-xs transition disabled:opacity-50 cursor-pointer"
            >
            ...
            <button
              ...
              className="inline-flex items-center gap-1.5 rounded-lg border border-blue-300 dark:border-blue-500/30 bg-blue-50 dark:bg-blue-900/20 hover:bg-blue-100 dark:hover:bg-blue-900/30 px-3 py-1.5 text-xs font-semibold text-blue-700 dark:text-blue-300 shadow-xs transition cursor-pointer"
            >
            ...
            {onSimulateClick && (
              <button
                ...
                className="inline-flex items-center gap-1.5 rounded-lg bg-[#2B6FFF] hover:bg-[#2055CC] px-3 py-1.5 text-xs font-bold text-white shadow-sm transition cursor-pointer active:scale-[0.99]"
              >
        ...
        {testResult && (
          ...
          >
            ...
            <button
              ...
              className="text-xs font-semibold underline opacity-80 hover:opacity-100 ml-2 cursor-pointer"
            >
              Dismiss
            </button>
          </div>
          ...
          <div className="w-full max-w-xl rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-[#131824] shadow-2xl overflow-hidden">
            ...
            <div className="p-6 space-y-4 max-h-[75vh] overflow-y-auto text-xs text-slate-700 dark:text-slate-300">
              <div className="space-y-1.5">
                <p className="font-bold text-slate-900 dark:text-slate-100">1. Webhook Ingestion Endpoint</p>
              ...
              <div className="space-y-1.5">
                <p className="font-bold text-slate-900 dark:text-slate-100">2. Expose Localhost via ngrok</p>
              ...
              <div className="space-y-1.5">
                <p className="font-bold text-slate-900 dark:text-slate-100">3. Configure in Razorpay Merchant Dashboard</p>
                <ol className="list-decimal pl-4 space-y-1.5 text-slate-600 dark:text-slate-400">
                  <li>Log in to <a href="https://dashboard.razorpay.com" target="_blank" rel="noreferrer" className="text-blue-600 dark:text-blue-400 font-bold underline inline-flex items-center gap-0.5">Razorpay Dashboard <ExternalLink className="h-2.5 w-2.5" /></a> and switch to <strong>TEST MODE</strong>.</li>
